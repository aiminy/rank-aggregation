...
sgdBT2 = function(data, mu, sigma, rate, maxiter = 1000, tol = 1e-9, start){
  #browser()
  ...
  while(flag){
    ...
    for(i in 1:nobs){
      ...
      res_temp = targetBT2(score_temp, adherence_temp, data, mu, sigma)
...
res = sgdBT2(data, sigma = diag(rep(1, 99)), rate = 0.1, 
            mu = rep(0, 99), start = rep(1, 198), maxiter = 10000, tol = 1e-9)

#plot to see if the function value decreases
plot(1:length(res[[1]]), res[[1]], type = 'l')

#estimated scores after 2911 iterations
est_score = res$score
